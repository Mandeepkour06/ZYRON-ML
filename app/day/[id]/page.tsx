    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/dashboard" className="text-blue-500 hover:text-blue-400 font-medium">
            ← Back to Dashboard
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-black">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <h1 className="text-3xl font-bold text-white mb-4">Challenge Locked</h1>
    ...
    <div className="min-h-screen bg-black">
      ...
      {showToast && (
        <div className="fixed top-4 right-4 z-50 animate-slide-in">
          <div className="bg-gray-900 border border-blue-500 rounded-lg p-4 shadow-lg max-w-md">
      ...
      {showXPAnimation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="text-center animate-bounce">
            ...
            <div className="text-6xl font-bold text-yellow-400 mb-2">+{challenge.xpReward} XP</div>
            <div className="text-2xl text-white">Challenge Complete!</div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            <span className={`px-4 py-2 rounded-lg text-sm font-medium border ${getDifficultyColor()}`}>
            ...
            <span className="px-4 py-2 bg-yellow-500/20 text-yellow-400 rounded-lg text-sm font-medium border border-yellow-500/30">
          ...
          <p className="text-gray-300 text-lg leading-relaxed mb-6">{challenge.description}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
              ...
              <div className="space-y-6">
                {challenge.instructions.map((instruction) => (
                  <div key={instruction.step} className="border-l-4 border-blue-500 pl-6">
                    <div className="flex items-start gap-3 mb-2">
                      ...
                      <h3 className="text-xl font-semibold text-white mt-1">{instruction.title}</h3>
                    </div>
                    ...
                    {instruction.code && (
                      <pre className="bg-black border border-gray-700 rounded-lg p-4 overflow-x-auto">
                        <code className="text-green-400 text-sm">{instruction.code}</code>
                      </pre>
                    )}
                  </div>
                ))}
            ...
            <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
              ...
              <div className="space-y-3">
                {challenge.requirements.map((req) => (
                  ...
                  >
                    ...
                    />
                    <span className={`flex-1 ${checkedRequirements[req.id] ? 'line-through text-gray-500' : 'text-gray-300'}`}>
                      {req.text}
                    </span>
                  </label>
                ))}
              ...
              {allRequirementsChecked && (
                <div className="mt-4 p-3 bg-green-500/20 border border-green-500 rounded-lg text-center">
                  <span className="text-green-400 font-medium">🎉 All requirements checked! Ready to complete.</span>
                </div>
              )}
            </div>
                  ...
                  <div className="flex gap-4">
                    {(githubUrl || liveUrl) && (
                      <button
                        ...
                        className="flex-1 px-6 py-4 bg-blue-600 text-white rounded-lg font-semibold text-lg hover:bg-blue-700 transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed"
                      >
                    ...
                    <button
                      ...
                      className="flex-1 px-6 py-4 bg-green-600 text-white rounded-lg font-semibold text-lg hover:bg-green-700 transition-colors"
                    >
                      ✓ Mark as Completed
                    </button>
                  </div>
              ...
              <div className="bg-green-500/20 border border-green-500 rounded-lg p-8 text-center">
                ...
                {studentProgress.submissions[dayId] && (
                  <div className="text-left max-w-md mx-auto mb-6">
                    <p className="text-gray-400 text-sm mb-2">Completed: {new Date(studentProgress.submissions[dayId].completedAt).toLocaleString()}</p>
                    {studentProgress.submissions[dayId].githubUrl && (
                      <p className="text-gray-400 text-sm">GitHub: {studentProgress.submissions[dayId].githubUrl}</p>
                    )}
                    {studentProgress.submissions[dayId].liveUrl && (
                      <p className="text-gray-400 text-sm">Live: {studentProgress.submissions[dayId].liveUrl}</p>
                    )}
                  </div>
                )}
                <Link
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3">
                {challenge.resources.map((resource, i) => (
                  <a
                    ...
                    className="block p-3 bg-gray-800 rounded-lg hover:bg-gray-750 transition-colors border border-gray-700 hover:border-blue-500"
                  >
                    <div className="flex items-start gap-3">
                      <span className="text-2xl">{getResourceIcon(resource.type)}</span>
                      <div className="flex-1">
                        <h3 className="font-medium text-white text-sm mb-1">{resource.title}</h3>
            ...
            {/* Challenge Info */}
            <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
              <h2 className="text-xl font-bold text-white mb-4">Challenge Info</h2>
